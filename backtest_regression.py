...
def backtest_regression():
    ...
    # signal: buy if predicted_return > threshold
    threshold = getattr(BacktestConfig, 'REGRESSION_THRESHOLD', 0.0)
    df['signal'] = df['predicted_return'].apply(lambda x: 1 if x > threshold else 0)

    # convert signals into buy_map where we optionally also allow explicit sell signals later (none here)
