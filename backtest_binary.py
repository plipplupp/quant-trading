# =============================================================================
# backtest_binary.py - Uppdaterad version
# =============================================================================
import os
import joblib
import pandas as pd
...
def backtest_binary():
    print("\n--- Backtest Binary (med säljsignaler) ---")
    ...
    def generate_binary_signal(prob):
        if prob > BacktestConfig.BINARY_THRESHOLD_BUY:
            return 1   # Köp
        elif prob < BacktestConfig.BINARY_THRESHOLD_SELL:
            return -1  # Sälj
        else:
            return 0   # Håll/neutral

    df['signal'] = df['pred_prob'].apply(generate_binary_signal)
    ...
    print(f"Tröskelvärden: Köp > {BacktestConfig.BINARY_THRESHOLD_BUY}, Sälj < {BacktestConfig.BINARY_THRESHOLD_SELL}")
    ...
    latest_df[['ticker', 'recommendation', 'pred_prob']].to_csv(signals_out, index=False)
